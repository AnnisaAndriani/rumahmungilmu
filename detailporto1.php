...
<li class="breadcrumb-item active" aria-current=page>Portofolio Blossom</li>
</ol>
</nav>
</div>
<div class="col-md-6 mb-4 mb-md-0">
<div class=preview-scroll-wrapper>
<img id=previewImage src=assets/img/blossom.jpeg alt="Preview Blossom" class=preview-img>
</div>
</div>
<div class=col-md-6>
<h1 class="fw-bold mb-1" style=color:#59b6fdff>Portofolio Blossom</h1>
...
<button class="btn btn-outline-danger-custom color-btn active" onclick='changeImage("pink",this)'>Pink</button>
<button class="btn btn-outline-danger-custom color-btn" onclick='changeImage("cyan",this)'>Hijau</button>
<button class="btn btn-outline-danger-custom color-btn" onclick='changeImage("lilac",this)'>Biru</button>
...
<script>function changeImage(e,s){document.getElementById("previewImage").src={pink:"assets/img/blossom.jpeg",cyan:"assets/img/Screenshot_20-7-2025_19274_localhost.jpeg",lilac:"assets/img/Screenshot_20-7-2025_193126_localhost.jpeg"}[e],document.querySelectorAll(".color-btn").forEach((e=>e.classList.remove("active"))),s.classList.add("active")}</script>
<script>function customColorWhatsApp(){const a=`https://wa.me/6282281980530?text=${encodeURIComponent("Halo! Saya tertarik dengan etalase Portofolio Blossom.\nBoleh tanya dulu, apakah bisa request warna berikut:\nWarna: (isi warna yang diinginkan)\n\nTerima kasih! Saya tunggu informasi dari rumah mungilmu")}`;window.open(a,"_blank")}</script>
<script>function pesanSekarang(){const n=`https://wa.me/6282281980530?text=${encodeURIComponent('Halo, saya ingin memesan Portofolio Blossom.\n\nNama:\nNo WhatsApp:\nWarna: (jika pesan warna custom isi dengan "custom")\n\nTerima kasih, saya tunggu informasinya ya')}`;window.open(n,"_blank")}</script>
</body>